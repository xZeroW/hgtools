...
export default function Magus() {
  ...
  function handlePbar() {
    if (pbar < 55){
      setPbar(pbar+1);
      magpbar.current.style.height = pbar * 1.85 + '%';
    }
  }
        ...
        <tr>
          <td rowSpan="5">
            <div className="progress progress-bar-vertical" onClick={handlePbar}>
              <div className="progress-bar" id="magpbar" ref={magpbar} role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="55">{pbar}
              </div>
            </div>
          </td>
