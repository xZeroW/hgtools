...
export default function Crusader() {
  ...
  function handlePbar() {
    if (pbar < 55){
      setPbar(pbar+1);
      crupbar.current.style.height = pbar * 1.85 + '%';
    }
  }
  ...
  return(
    <table>
      <tbody>
        <tr>
          <td rowSpan="5">
            <div className="progress progress-bar-vertical" onClick={handlePbar}>
              <div className="progress-bar bg-danger" id="crupbar" ref={crupbar} role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="55">{pbar}
              </div>
            </div>
          </td>
        ...
        <tr>
          <td onClick={() => setSkills({...skills})}><img src="https://via.placeholder.com/42" alt="skill"></img><div>{skills[0]}</div></td>
          <td></td>
